...
def expand_words_dimension_mean(
    ...
    """
    vocab_number = word2vec_model.wv.vectors.shape[0]
    all_vocab = set(word2vec_model.wv.index_to_key)
    expanded_words = {}
    filtered_words = {}
    all_seeds = set()
    ...
    for dimension in seed_words:
        dimension_words = [word for word in seed_words[dimension] if word in all_vocab]
        ...
        if filter_word_set is not None:
            filtered_words[dimension] = [
                x for x in similar_words if (x in filter_word_set)
            ]
    ...
    return expanded_words, filtered_words


def rank_by_sim(expanded_words, seed_words, word2vec_model) -> "dict[str: list]":
    ...
    for dimension in expanded_words.keys():
        dimension_seed_words = [
            word for word in seed_words[dimension] if word in all_vocab
        ]
        similarity_dict = dict()
        for w in expanded_words[dimension]:
            if w in all_vocab:
                similarity_dict[w] = word2vec_model.wv.n_similarity(
...
def write_dict_to_csv(expanded_dict, file_name):
    """write the expanded dictionary to a csv file, each dimension is a column, the header includes dimension names

    Arguments:
        expanded_dict {dict[str, list[str]]} -- an expanded dictionary {dimension: [words]}
        file_name {str} -- where to save the csv file?
    """
    pd.DataFrame.from_dict(expanded_dict, orient="index").transpose().to_csv(
        file_name, index=None
    )


def read_dict_from_csv(file_name):
    """Read expanded dict from a csv file
    ...
    for k in expanded_dict.keys():
        expanded_dict[k] = set([x for x in expanded_dict[k] if x == x])  # remove nan
...
def deduplicate_keywords(word2vec_model, expanded_words, seed_words):
    ...
    for dimension in expanded_words:
        expanded_words[dimension] = set(expanded_words[dimension]).difference(dup_words)
...
def score_one_document_tf(document, expanded_words, list_of_list=False):
    ...
    if list_of_list is False:
        document = Path(document).read_text().split()
...
def score_tf_idf(
    ...
    results = []
    for i, document in tqdm(enumerate(documents), total=len(document_ids)):
        dimension_count = OrderedDict()
        for dimension in expanded_words:
            dimension_count[dimension] = 0
        c = Counter(document)
            ...
            for dimension, words in expanded_words.items():
                if pair[0] in words:
                    if method == "TF":
                        ...
                        raise Exception(
                            "The method can only be TF, TFIDF, WFIDF, TFIDF+SIMWEIGHT, or WFIDF+SIMWEIGHT"
                        )
...
def compute_word_sim_weights(file_name):
    ...
    sim_weights = {}
    for k in expanded_dict.keys():
        expanded_dict[k] = [x for x in expanded_dict[k] if x == x]  # remove nan
    for key in expanded_dict:
        for i, w in enumerate(expanded_dict[key]):
